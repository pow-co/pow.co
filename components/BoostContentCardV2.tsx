...
const BoostContentCardV2 = ({ content_txid, difficulty, rank }: Ranking) => {
    ...
    const avatar = useMemo(() => {
        switch (true){
            ...
            case paymail?.includes('twetch'):
                return `https://auth.twetch.app/api/v2/users/${paymail.split('@')[0]}/icon`
            case paymail?.includes('handcash'):
                return `https://cloud.handcash.io/v2/users/profilePicture/${paymail.split('@')[0]}`
            default:
                return ''
        }
    },[paymail])
        ...
        if (content.bmap){
            ...
            switch (true){
                case content.content_type?.includes("application/json"):
                    // onchainsv
                    let onchainData = await axios.get(`https://onchain.sv/api/v1/events/${content_txid}`)
    ...
    const getData = async () => {
        const [contentResult, repliesResult] = await Promise.all([
            ...
            axios.get(`${BASE}/content/${content_txid}/replies`)
                .catch((err) => {
                ...
                <div className='col-span-11 ml-6'>
                    ...
                    {linkUnfurls.map((linkUnfurl: any) => (
                        ...
                        >
                            ...
                            />
                            <div className={`bg-primary-300 dark:bg-primary-700 py-3 px-4 ${linkUnfurl.image? "rounded-b-xl" : "rounded-xl"}`}>
                                <div className='h-[46px] flex flex-col justify-center'>
                                    <p className='text-sm font-semibold whitespace-nowrap overflow-hidden text-ellipsis'>{linkUnfurl.description}</p>
                                    <p className='text-xs mt-2 whitespace-nowrap overflow-hidden text-ellipsis'>{linkUnfurl.url}</p>
                                </div>
                            </div>
                        </a>
                    ))}
