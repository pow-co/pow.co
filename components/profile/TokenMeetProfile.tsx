	...
	return (
		<div id='right-sidebar'>
		  <div className='fixed top-[102px] z-50 w-[344px]'>
			<h2 className="text-4xl font-bold tracking-tight text-gray-900 sm:text-4xl">Here shall be recent posts tagged #{channel}</h2>
		  </div>
		</div>
	)
}
...
export default function TokenMeetProfile({channel}: {channel: string}) {
  ...
  useEffect(() => {
    ...
    axios.get(`https://api.tokenmeet.live/api/v1/channels/${channel}`).then((data) => {
  ...
  useEffect(() => {
    ...
    axios.get(`https://api.tokenmeet.live/api/v1/shows/${channel}`).then((data) => {
  ...
  return (
    <PanelLayout>
      <div className="mb-[200px] min-h-screen ">
	    ...
	    return <EpisodeDashboard key={episode.id} loading={loading} episode={episode}/>

          })} 

          {videos.map((video: any) => {
	    if (!video.playback.hls_url) { return <></>}
	    return <EpisodeDashboard key={video.id} loading={loading} episode={{hls_playback_url: video.playback.hls_url}}/>
          })} 
